...
Examples:
  ...
  winrar-installer.exe --cli --beta              # 🧪 Show beta versions
    ...
    parser.add_argument('--install', action='store_true',
                       help='Install WinRAR (CLI mode only)')
    parser.add_argument('--version', type=str,
                       help='Specify WinRAR version to install (e.g., 7.11)')
...
def run_cli(args):
    ...
    print(f"{Fore.GREEN}🚀 Installing WinRAR {Fore.YELLOW}{version} {Fore.GREEN}({Fore.CYAN}{args.language}{Fore.GREEN})")
            ...
            for chunk in response.iter_content(chunk_size=8192):
                if chunk:
                    ...
                    if total_size > 0:
                        ...
                        print(f"\r{Fore.CYAN}📊 Progress: {Fore.YELLOW}[{bar}] {progress:.1f}%", end='', flush=True)
